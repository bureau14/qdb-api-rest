...
writer, gzip, and the e2e `text/csv` full-table target.
...
| `float64`         | bare number, shortest round trip, `encoding/json`'s rule | same text                              |
| `float64` NaN/Inf | `null`                                                   | empty field                            |
| `timestamp[ns]`   | `"2026-06-11T00:00:00.000683000Z"`                       | same text, unquoted                    |
| `utf8`            | JSON string (`jsontext.AppendQuote`)                     | RFC 4180 quoting; empty string is `""` |
...
- `float64` follows `encoding/json`: `strconv` shortest round trip,
  plain notation for exponents in [-6, 21), exponent notation outside.
  NaN and the infinities are not JSON, and NaN is QuasarDB's own null
  for doubles at the writer, so null is the honest reading.
...
- A field is quoted only when it holds a comma, a quote, a CR or an LF,
  `encoding/csv`'s rule. A null is the empty field; an empty string is
  `""`, so the two stay distinguishable at zero cost (DuckDB reads them
  apart; pandas folds both to NaN, as it does for any CSV).
- A nil batch is an empty body: no columns means no header line.
...
- `cell.go`: the shared vocabulary. The Arrow-type-to-word function; the
  timestamp and base64 renderers; the float rule. Dense code, so it
  carries the walk-through comments (`internal/AGENTS.md`, Code).
- `json.go`: `JSON` and `NDJSON`. Both write bytes through one
  `bufio.Writer` from `strconv.Append*` and `jsontext.AppendQuote` into
  a scratch buffer reused per row; no encoder object, no per-cell
  allocation. The type switch runs once per column and yields a
  per-column cell appender, so the hot loop is a call per cell. The ctx
  is checked at a row stride, as the Arrow encoder checks it per batch.
...
| Streaming JSON through `strconv` appenders and `AppendQuote` | owner decision; no per-cell allocation on the hot loop; the standard library's escaper          | `json/v2` `jsontext.Encoder` (token state per cell); hand-rolled escaping |
...
| No alignment with `qdb_export`'s CSV                         | owner decision; the tool exports raw tables, not queries; user-friendliness and speed decide    | always-quoted strings, no header, nine-digit naive timestamps             |
| Header row, LF, minimal quoting, `""` for the empty string   | what every reader accepts and what keeps null and empty apart at no cost                        | CRLF; no header; always-quoting                                           |
